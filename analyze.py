...
expenses = [dict(zip(csv_headers, row)) for row in csv_body]
...
for expense in expenses:
    ...
    if not [new_expense[1],new_expense[2],new_expense[3]] in cells_for_dups: #if 'new_expense' doesn't have 1,2 and 3 keys, there is no enough info to place the record, so an exception will be raised /Idan.
